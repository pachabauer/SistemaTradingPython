    ...
    def stop(self):
        ...
        if ticker not in self.best_results or final_value > self.best_results[ticker]['final_pnl']:
            self.best_results[ticker] = {
                ...
                '% winning_trades': self.winning_trades / self.total_trades if not ZeroDivisionError else 0,
                '% losing_trades': self.losing_trades / self.total_trades if not ZeroDivisionError else 0,
